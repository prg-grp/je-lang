...
abstract class InformationFlowTypeVisitor[TypeFactory <: GenericAnnotatedTypeFactory[_, _, _, _]](checker : BaseTypeChecker) extends BaseTypeVisitor[TypeFactory](checker) {
	...
	protected val implicitContext : ImplicitContext = new ImplicitContext  // From Mirko

	protected var contextStack = mutable.Stack[AnnotationMirror]()
	...
	override def visitIf(node : IfTree, p : Void) : Void = {
		val conditionAnnotation : AnnotationMirror = getPrivacyAnnotation(node.getCondition)
	...
	override def visitEnhancedForLoop(node : EnhancedForLoopTree, p : Void) : Void = { //TODO: add variable to implicit context?
		val conditionAnnotation : AnnotationMirror = getPrivacyAnnotation(node.getExpression)
	...
	private def getPrivacyAnnotation(node : ExpressionTree) : AnnotationMirror = {
		/*
